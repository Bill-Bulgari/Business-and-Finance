...
def perceptron_single_step_update(feature_vector, label, current_theta, current_theta_0):
    ...
    """
    x = feature_vector; y = label; theta = current_theta; theta_0 = current_theta_0
    score = x @ theta + theta_0
    if y * score <= 0:
        current_theta = theta + y * x
        current_theta_0 = theta_0 + y 
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    X = feature_matrix; y = labels; n, p = X.shape; theta = np.zeros(p); theta_0 = 0
    for t in range(T):
        for i in get_order(n):
            theta, theta_0 = perceptron_single_step_update(X[i, :], y[i], theta, theta_0)
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    X = feature_matrix; y = labels; n, p = X.shape; theta = np.zeros(p); theta_0 = 0
    sum = 0; sum_0 = 0; count = n*T
    for t in range(T):
        for i in get_order(n):
            theta, theta_0 = perceptron_single_step_update(X[i, :], y[i], theta, theta_0)
            sum += theta; sum_0 += theta_0
    theta = sum/count; theta_0 = sum_0/count
    return theta, theta_0
...
def pegasos_single_step_update(feature_vector, label, L, eta, theta, theta_0):
    ...
    Returns:
        ...
        real valued number with the value of theta_0 after the old update has
        completed.
    """
    x = feature_vector; y = label; score = x @ theta + theta_0
    if y * score <= 1:
...
def pegasos(feature_matrix, labels, T, L):
    ...
    """
    X = feature_matrix; y = labels; n, p = X.shape 
    theta = np.zeros(p); theta_0 = 0; count = 0
    for t in range(T):
        for i in get_order(n):
            count += 1; eta = 1/np.sqrt(count)
            theta, theta_0 = pegasos_single_step_update(X[i, :], y[i], L, eta, theta, theta_0)
    return theta, theta_0
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    X = feature_matrix; scores = X @ theta + theta_0
    predictions = 2 * np.heaviside(scores, 0) - 1
    return predictions 
